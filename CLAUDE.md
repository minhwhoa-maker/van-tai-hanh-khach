...
- `manifest-hang.html` — chọn 1 chuyến, xem kiện gom theo tỉnh (thứ tự theo `tinh_tuyen.thu_tu`, chiều lấy từ `chuyen.chieu`: bac = ASC, nam = DESC). Mỗi dòng kiện chuyển đổi tại chỗ trong cùng 1 `.kien-row` (không điều hướng trang) qua các hàm `renderKienRowView` / `renderKienRowEdit` / `renderKienRowThuTien` / `renderKienRowViTri`:
  ...
  - **Dưới 600px** (`@media (max-width: 600px)` trong `<style>` của trang): `.kien-row` chuyển `flex-wrap: wrap` — ảnh/tên điểm/badge giữ 1 hàng. `.btn-primary-action`/`.btn-menu-more` của `renderKienRowView` to hơn (padding/font lớn hơn) nhưng KHÔNG ép full-width — chỉ 1-2 nút nên đã đủ gọn để nằm cùng hàng ảnh/tên/badge. Riêng các form 2 nút Lưu/Hủy (`renderKienRowEdit`/`renderKienRowThuTien`/`renderKienRowViTri`, có class `.kien-row-edit`) vẫn giữ style cũ: `.kien-actions` xuống hàng riêng full-width, mỗi nút `.btn-sm` chiếm nửa hàng — 2 bộ style này tách riêng qua selector `.kien-row-edit .kien-actions`/`.kien-row-edit .btn-sm` để không áp nhầm lẫn nhau.
