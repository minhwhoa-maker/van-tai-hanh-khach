...
- `hang.html` — nhập kiện: chọn/tạo **chuyến** (chiều bắc/nam) → chọn tỉnh → chọn/tạo điểm → chụp ảnh + SĐT người nhận + ghi chú → lưu offline-first vào IndexedDB (`idb-queue.js`), tự đồng bộ khi có mạng
  ...
  - **Bước 3 có thêm nút "🔗 Liên kết kiện trả lại"** (`#btn-lienket-toggle`, style dùng chung `.btn-cod-toggle` nhưng màu trung tính `--text-muted` thay vì `--warning` — không phải hành động về tiền) — HOÀN TOÀN tùy chọn, phụ trợ audit cho luồng "Trả hàng" (xem `traHang`/`kien_goc_id` ở mục Database và `manifest-hang.html`). Bấm vào toggle hiện/ẩn `#lienket-wrap`, load danh sách kiện `trang_thai = 'tra_lai'` CHƯA bị link (`loadKienTraLaiChuaLienKet` — query phụ lấy hết `kien_goc_id` đã dùng để loại khỏi danh sách, tránh link trùng), render dạng `.diem-item` để chọn. Chọn 1 kiện → lưu vào biến `selectedKienGocId`/`selectedKienGocLabel`, hiện lại ở `#lienket-selected` (bấm vào để bỏ chọn), đóng danh sách. Lúc lưu: `record.kien_goc_id = selectedKienGocId` (null nếu không chọn) — có trong `idb-queue.js`. Reset về `null` sau khi lưu kiện (như `loaiHangItems`), khôi phục đúng giá trị cũ trong `lastSaved`/"Sửa lại kiện này" giống các field khác của bước 3. Không bấm gì thì hành vi y hệt trước khi có tính năng này.
...
- `manifest-hang.html` — chọn 1 chuyến, xem kiện gom theo tỉnh (thứ tự theo `tinh_tuyen.thu_tu`, chiều lấy từ `chuyen.chieu`: bac = ASC, nam = DESC). Mỗi dòng kiện chuyển đổi tại chỗ trong cùng 1 `.kien-row` (không điều hướng trang) qua các hàm `renderKienRowView` / `renderKienRowEdit` / `renderKienRowThuTien` / `renderKienRowViTri`:
  ...
  - **`traHang`** — đánh dấu 1 kiện `chua_giao` thành `tra_lai` (đã tới điểm giao nhưng không giao được), cùng pattern với `huyDon`: `confirmDialog()` (cảnh báo thêm nếu `demSoLanThuLienLac(k.ghi_chu) === 0`, tức chưa "Ghi nhận đã gọi" lần nào — vẫn cho tiếp tục nếu crew xác nhận, không chặn cứng) rồi `UPDATE kien.trang_thai = 'tra_lai'`, gọi lại `loadManifest(chuyen)`. Chỉ hiện trong menu "⋯" khi `chua_giao` (không cho trả hàng 1 kiện đã giao — phải "Hủy giao" về `chua_giao` trước, giống ràng buộc của "Hủy đơn"). Xem bullet `trang_thai 'tra_lai'` ở mục Database để biết toàn bộ luồng nghiệp vụ (mang hàng về ở chuyến ngược, `kien_goc_id`).
...
kien       (id uuid PK — CLIENT TỰ SINH qua crypto.randomUUID() để offline-first,
           ...
           -- `huyDon`: UPDATE rồi refetch `loadManifest` toàn bộ). Bị loại khỏi manifest active
           -- + mọi tổng COD/tổng tiền qua CÙNG filter với 'huy' — `.not('trang_thai', 'in',
           -- '(huy,tra_lai)')` ở `manifest-hang.html loadManifest` và `lich-su-chuyen.html`
           -- (trước đây 2 chỗ này dùng `.neq('trang_thai', 'huy')`, đổi sang `.not(...'in'...)`
           -- để loại cả 2 giá trị cùng lúc). KHÔNG có cơ chế tự động chuyển kiện qua chuyến
           ...
           -- `nam`), tùy chọn gắn `kien_goc_id` trỏ về kiện `tra_lai` gốc để giữ dấu vết audit
           -- qua nút "🔗 Liên kết kiện trả lại" ở bước 3 (xem bullet riêng ở `hang.html`).
           -- KHÔNG giới hạn cứng số lần thử liên lạc trước khi được phép "Trả hàng" (quyết định
           ...
           -- kien_goc_id: thêm sau v1 (alter table), nullable, tự tham chiếu `kien(id)` — CHỈ
           -- set khi tạo 1 kiện dạng "mang hàng trả về" ứng với 1 kiện `tra_lai` cụ thể, qua nút
           -- "🔗 Liên kết kiện trả lại" ở `hang.html` bước 3 (HOÀN TOÀN tùy chọn, không bấm thì
           -- để null như kiện thường — tính năng phụ trợ audit, không chặn luồng lưu chính).
           -- Danh sách chọn chỉ hiện kiện `trang_thai = 'tra_lai'` CHƯA có kiện nào khác link
           -- tới (tránh link trùng 1 kiện trả lại cho 2 kiện "mang về" khác nhau) — lọc bằng 1
           -- query phụ lấy hết `kien_goc_id` đã dùng rồi loại khỏi danh sách hiển thị. Không có
           -- badge/UI đặc biệt đánh dấu "đây là hàng trả lại" ở `manifest-hang.html` cho v1 — có
           -- cột này là đủ để audit qua Supabase dashboard khi cần đối chiếu.
