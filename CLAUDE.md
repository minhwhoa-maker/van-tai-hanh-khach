...
- `lich-su-chuyen.html` — liệt kê chuyến đã `trang_thai = 'xong'` (`order by khoi_hanh desc limit 50`), mỗi card show `X kiện · Đã thu: Yđ` (tổng `tien_thu` các kiện của chuyến, tính ở client — 1 query `kien` duy nhất với `.in('chuyen_id', ...)` cho cả 50 chuyến rồi filter/group theo `chuyen_id`, không dùng RPC/view riêng vì data nhỏ) và cảnh báo "⚠ Còn N kiện chưa thu đủ COD" nếu có. Bấm vào card điều hướng sang `manifest-hang.html?chuyen_id=<id>` (xem preselect ở trên).
